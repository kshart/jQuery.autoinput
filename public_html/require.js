...
var AutoInput = function () {
	function AutoInput(el, options) {
		...
		_classCallCheck(this, AutoInput);
		...
		if (val.serverType === "local") {
			...
			if (typeof val.serverOnEndLoad === "function") {
				this.server.xhr.onload = function () {
					val.serverOnEndLoad();
					_this.serverLoadedItems();
				};
			} else {
				this.server.xhr.onload = function () {
					_this.serverLoadedItems();
			...
			if (typeof val.serverOnStartLoad === "function") {
				this.server.xhr.onloadstart = function () {
			...
			if (typeof val.serverOnError === "function") {
				this.server.xhr.ontimeout = this.server.xhr.onabort = this.server.xhr.onerror = function () {
					val.serverOnError();
				};
			}
		}
	...
	_createClass(AutoInput, [{
		key: "clientLimitUpdate",
		value: function clientLimitUpdate(el, limit) {
			var _this2 = this;

			$("[data-ai-limit-page]", el).text(limit.page);
			$("[data-ai-limit-page-count]", el).text(limit.pageCount);
			$("[data-ai-limit-prev]", el).on("click", function () {
				_this2.clientLimitPrevPage();
			});
			$("[data-ai-limit-next]", el).on("click", function () {
				_this2.clientLimitNextPage();
			});
		}
	}, {
		key: "clientLimitPrevPage",
		value: function clientLimitPrevPage() {
			if (this.lastRequest === undefined || this.clientLimit.page === 1) return;
			--this.clientLimit.page;
			this.sendRequest(this.lastRequest);
		}
	}, {
		key: "clientLimitNextPage",
		value: function clientLimitNextPage() {
			if (this.lastRequest === undefined || this.clientLimit.page === this.clientLimit.pageCount) return;
			++this.clientLimit.page;
			this.sendRequest(this.lastRequest);
		}
	}, {
		key: "clientLimitGoPage",
		value: function clientLimitGoPage(page) {
		...
		value: function beforeUpdate() {
			...
			if (this.beforeUpdateTimerID === undefined) {
				this.beforeUpdateTimerID = setTimeout(function () {
					_this3.update();
				}, this.clientAutoUpdateTimeoutMS);
			}
		}
	}, {
		key: "update",
		...
		value: function sendRequest(request) {
			if (this.server === undefined) {
				...
				var result = this.localRequest(request, limit),
				    el = $("[data-ai-output]", this.el).first(),
				    elLimit = void 0;
			...
			if (this.server.cache) for (var i in request) {
			...
			if (this.server.cache) {
				...
				if (cacheValue !== null && cacheValue !== undefined) {
					var _el = $("[data-ai-output]", this.el).first(),
					    _elLimit = void 0;
					if (_el[0] !== undefined) {
						this.clientViewData(_el[0], cacheValue);
						_elLimit = $("[data-ai-limit]", this.el).first();
						if (_elLimit[0] !== undefined) this.clientLimitUpdate(_elLimit[0], this.clientLimit);
					}
				}
			}
			...
			if (this.server.method === "POST") {
				...
				if (this.server.sendArgType) {
					...
					for (var _i in request) {
						requestWithoutArgType[_i] = { value: request.value };
					}
				...
				var getStr = "?";
				if (this.server.sendArgType) {
					for (var _i2 in request) {
						switch (request[_i2].type) {
							case "checkbox":
								getStr += encodeURIComponent(_i2) + "=" + (request[_i2].value === true ? "1&" : "0&") + _i2 + "type=" + encodeURIComponent(request[_i2].type) + "&";
								break;
							case "text":
							case "radio":
							default:
								getStr += encodeURIComponent(_i2) + "=" + encodeURIComponent(request[_i2].value) + "&" + _i2 + "type=" + encodeURIComponent(request[_i2].type) + "&";
						}
					}
				} else {
					for (var _i3 in request) {
						switch (request[_i3].type) {
							case "checkbox":
								getStr += encodeURIComponent(_i3) + "=" + (request[_i3].value === true ? "1" : "0") + "&";
								break;
							case "text":
							case "radio":
							default:
								getStr += encodeURIComponent(_i3) + "=" + encodeURIComponent(request[_i3].value) + "&";
						}
					}
				}
		...
		key: "serverLoadedItems",
		value: function serverLoadedItems() {
			var response;
			...
			if (_typeof(response.limit) === "object") {
				if (typeof response.limit.pageCount === "number" || typeof response.limit.page === "number" || typeof response.limit.itemsOnPage === "number") {
			...
			var el = $("[data-ai-output]", this.el).first(),
			    elLimit;
			if (el[0] !== undefined) {
			...
			if (this.server.cache) {
				if (sessionStorage.length > 100) {
					for (var i = 0, key; i < sessionStorage.length; ++i) {
						...
						if (key[0] !== "a" || key[1] !== "i" || key[2] !== ":") continue;
						sessionStorage.removeItem(key);
					}
				}
				var requestStr = "";
				for (var _i4 in this.lastRequest) {
					switch (this.lastRequest[_i4].type) {
						case "checkbox":
							requestStr += _i4 + this.lastRequest[_i4].type + (this.lastRequest[_i4].value === true ? "1" : "0");
							break;
						case "text":
						case "radio":
						default:
							requestStr += _i4 + this.lastRequest[_i4].type + this.lastRequest[_i4].value;
					}
				}
				if (this.clientLimit.itemsOnPage > 0) {
...
var _AutoInput = require('./AutoInput');
...
(function (factory) {
	...
	$.fn.AutoInput = function (obj, arg1, arg2) {
		...
		if (instance === undefined) {
			el.data(key, new _AutoInput.AutoInput(el, obj));
...
},{"./AutoInput":1,"jquery":4}],3:[function(require,module,exports){
'use strict';
...
(function (factory) {
	...
	$("#autoinput").AutoInput({
		localRequest: function localRequest(e, limit) {
			var arr = [{ value: 1, tag: "isRed" }, { value: 2, tag: "isBlue" }, { value: 3, tag: "isGreen isBlue" }],
			    result = [];
			if (limit === undefined) {
		...
		serverMethod: "GET",
		serverSendArgType: false,
		serverOnStartLoad: function serverOnStartLoad() {
			console.log("serverOnStartLoad");
		},
		serverOnEndLoad: function serverOnEndLoad() {
			console.log("serverOnEndLoad");
		},
		serverOnProgressLoad: function serverOnProgressLoad(e) {
			console.log("serverOnProgressLoad", e);
		},
		serverOnError: function serverOnError() {
			console.log("serverOnError");
		},
		clientLimitViewCount: 1,
		clientAutoUpdate: true,
		clientAutoUpdateTimeoutMS: 1000,
		clientViewData: function clientViewData(el, data) {
